    ...
    const bool writeOutput)
{
    ...
    MKL_INT nrhs = K_NUM_RHS;         // Number of right hand sides
    ...
    // Setup the user provided permutation if needed
    int numEquations = XDIM * YDIM * ZDIM;
    // std::vector<int> perm(numEquations);
    ...
    // Setup x and b
    float *xMultiple = new float[numEquations * K_NUM_RHS];
    float *bMultiple = new float[numEquations * K_NUM_RHS];

    for (int k = 0; k < K_NUM_RHS; k++) {
        for (int i = 0; i < numEquations; i++) {
            int first = i / (XDIM * YDIM);
            int second = (i / XDIM) % YDIM;
            int third = i % ZDIM;
            xMultiple[i + k*numEquations] = x[first][second][third];
            bMultiple[i + k*numEquations] = f[first][second][third];
        }
    }
